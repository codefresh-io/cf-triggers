...
func (_m *MockTriggerEventReaderWriter) CreateEvent(ctx context.Context, eventType string, kind string, secret string, _a4 string, values map[string]string) (*Event, error) {
	ret := _m.Called(ctx, eventType, kind, secret, _a4, values)

	var r0 *Event
	if rf, ok := ret.Get(0).(func(context.Context, string, string, string, string, map[string]string) *Event); ok {
		r0 = rf(ctx, eventType, kind, secret, _a4, values)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*Event)
		}
	...
	if rf, ok := ret.Get(1).(func(context.Context, string, string, string, string, map[string]string) error); ok {
		r1 = rf(ctx, eventType, kind, secret, _a4, values)
	} else {
		r1 = ret.Error(1)
	}
